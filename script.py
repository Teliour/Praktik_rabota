...
def on_parse():
    parse_logs()
    messagebox.showinfo("Информация", "Логи распарсены.")
...
ttk.Label(mainframe, text="Конец временного отрезка (YYYY-MM-DD):").grid(column=1, row=2, sticky=tk.W)
end_date_entry = ttk.Entry(mainframe, width=20)
end_date_entry.grid(column=2, row=2, sticky=(tk.W, tk.E))

ttk.Label(mainframe, text="IP адрес:").grid(column=1, row=3, sticky=tk.W)
